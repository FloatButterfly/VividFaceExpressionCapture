...
public class FaceHelper {
    ...
    public static Bitmap genFaceBitmap(Bitmap sourceBitmap) {
        ...
        if (DEBUG_ENABLE) {
            Log.i(TAG, "genFaceBitmap() : facefound - " + facefound);
        }
        if (0 == facefound) {
            if (DEBUG_ENABLE) {
                Log.e(TAG, "genFaceBitmap() : no face found , return null.");
            }
            return null;
        }
        ...
        if (DEBUG_ENABLE) {
            ...
            Log.i(TAG, "genFaceBitmap() : face center x - " + p.x
                    + " , y - " + p.y);
        }
        final int faceX = (int) p.x;
        final int faceY = (int) p.y;
        if (DEBUG_ENABLE) {
            Log.i(TAG, "genFaceBitmap() : int faceX - " + faceX
                    + " , int faceY - " + faceY);
        }
